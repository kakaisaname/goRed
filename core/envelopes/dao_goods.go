...
func (dao *RedEnvelopeGoodsDao) UpdateBalance(
	...
	sql := "update red_envelope_goods " +
		" set remain_amount=remain_amount-CAST(? AS DECIMAL(30,6)), " + //CAST转
		" remain_quantity=remain_quantity-1 " + //更新的红包都是减 1 ，在sql中写死
		" where envelope_no=? " +
		//最重要的，乐观锁的关键
...
//过期，把过期的所有红包都查询出来，分页，limit,offset size          过期红包可能会很多，需要做分页  ***
